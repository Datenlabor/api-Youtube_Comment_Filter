...
describe 'Acceptance Tests' do
  ...
  describe 'Homepage' do
    describe 'Visit Home page' do
      it '(HAPPY) should not see youtube_url if none created' do
        ...
        _(@browser.button(css: '.w-100.btn.btn-primary').present?).must_equal true # history
      end
  ...
  describe 'Project Page' do
    it '(HAPPY) should see youtube comments if the specific youtube history exists' do
      ...
      _(@browser.div(css: '.h-100.d-flex.flex-column.justify-content-center').present?).must_equal true
    end
  end
end
